...
void main(void)
{
  ...
  for (;;)
  {
    ...
    //while (1) {
       // transmitVowels(); // Continuously transmit vowels
     //   RTI_Delay_ms(250); // Delay 250 milliseconds between transmissions
   // }



    //while (1) {
       // TransmitVowelsWithSum(); // Continuously transmit vowels
     //   RTI_Delay_ms(250); // Delay 250 milliseconds between transmissions
   // }
  }
}
...
void TransmitVowelsWithSum()
{
  ...
  for (i = 0; i < 20; ++i)
  {
    int index = rand() % 10; // Random index for vowels
     if (isUpperCase) {
        str[i] = toupper(vowels[index]);
    } else {
        str[i] = tolower(vowels[index]);
    }
   sum += str[i];// Add character value to sum
...
{
  static int firstRun = 1;  // Flag to execute header printing only once
    char buffer[512];  // Large buffer to hold the entire message including ANSI codes
  int sum = 0;
  char vowels[] = "AEIOUaeiou";
  ...
  // On the first run, print the static header
   if (firstRun) {
        // Print the program header only on the first run
        sprintf(buffer, "\x1b[2J\x1b[0;0H\x1b[7mYour Name\x1b[0m\n");
        sci0_txStr(buffer);
        firstRun = 0;  // Prevent further header printing
    }
